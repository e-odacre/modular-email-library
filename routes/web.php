...
Route::get('/{library}', [StudioController::class, 'index'])
    ->whereIn('library', ['emails', 'recipes', 'components', 'brands', 'drafts'])->name('studio.library');
Route::get('/new-email', [WorkflowController::class, 'create'])->name('workflow.create');
Route::post('/new-email', [WorkflowController::class, 'store'])->name('workflow.store');
Route::get('/builds/{id}', [StudioController::class, 'show'])
    ->defaults('kind', 'draft')->where('id', '[a-z0-9-]+')->name('studio.draft');
...
Route::get('/frame/{kind}/{id}', [StudioController::class, 'frame'])
    ->whereIn('kind', ['email', 'recipe', 'component', 'draft'])->where('id', '[a-zA-Z0-9_/-]+')
...
Route::get('/brands/{brand}', [StudioController::class, 'brand'])
    ->where('brand', '[a-z0-9-]+')->name('studio.collection');
Route::get('/brand-designs/{brand}/{group}/{id}', [StudioController::class, 'brandItem'])
    ->where('brand', '[a-z0-9-]+')->whereIn('group', ['blocks', 'emails'])->where('id', '[a-z0-9-]+')->name('studio.collection.item');
